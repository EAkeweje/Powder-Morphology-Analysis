# Base O2 conv encoder adapted from https://github.com/QUVA-Lab/e2cnn_experiments/blob/master/experiments/models/exp_e2sfcnn.py
import numpy as np
import torch
...
class E2SFCNN(torch.nn.Module):
    def __init__(
        ...
        sgsize: int = None,
        flip: bool = True,
        last_pooling: str = "pointwise_adaptive_max_pool",
        **kwargs,
    ):
        ...
        c_dims = cnn_dims
        assert len(c_dims) == 6, (
            "this model has hardcoded 6 cnn layers ... cnn_dims "
        ...
        self.LAYER += 1
        eq_layers += layer_builder(r1, c_dims[0], 7, 1)

        # 24 px
        # Convolutional Layer 2
        self.LAYER += 1
        eq_layers += layer_builder(eq_layers[-1].out_type, c_dims[1], 5, 2, 2)

        # 12 px
        # Convolutional Layer 3
        self.LAYER += 1
        eq_layers += layer_builder(eq_layers[-1].out_type, c_dims[2], 5, 2)

        # 12 px
        # Convolutional Layer 4
        self.LAYER += 1
        eq_layers += layer_builder(eq_layers[-1].out_type, c_dims[3], 5, 2, 2)

        # 6 px
        # Convolutional Layer 5
        self.LAYER += 1
        eq_layers += layer_builder(eq_layers[-1].out_type, c_dims[4], 5, 2)

        # 6 px
        # Convolutional Layer 6
        self.LAYER += 1
        eq_layers += layer_builder(eq_layers[-1].out_type, c_dims[5], 5, 0, None, True)

        # Adaptive Pooling
        if last_pooling == "pointwise_adaptive_avg_pool":
            mpl = PointwiseAdaptiveAvgPool(eq_layers[-1].out_type, 1)
        else:
    ...
    def repr2(self):
        ...
        repr += f"Total number of parameters: {tot_param}"
        repr = "MODEL Architecture:\n"
        for i, (name, mod) in enumerate(self.named_modules()):
            params = sum([p.numel() for p in mod.parameters() if p.requires_grad])
            if (
                isinstance(mod, EquivariantModule)
                and isinstance(mod.in_type, FieldType)
                and isinstance(mod.out_type, FieldType)
            ):
                repr += f"\t{i: <3} - {name: <70} | {params: <8} | {mod.in_type.size: <4}- {mod.out_type.size: <4}\n"
            else:
    ...
    def build_layer_regular(
        ...
        if pooling is not None:
            if self.antialias > 0.0:
                pl = PointwiseMaxPoolAntialiased(
                    layers[-1].out_type, stride=pooling, sigma=self.antialias
                )
            elif self.antialias < 0.0:
                pl = PointwiseAvgPoolAntialiased(
                    layers[-1].out_type, stride=pooling, sigma=-self.antialias
                )
            else:
    ...
    def build_layer_hnet_normpool(
        ...
        if self.fix_param and not invariant_map and self.LAYER > 1:
            ...
            size = sum(
                int(irr.size // irr.sum_of_squares_constituents)
                for irr in gc.fibergroup.irreps.values()
            )
            C = int(round(C / size))
    ...
    def build_layer_squash(
        ...
        return self.build_layer_hnet_conv2triv(
            r1,
            C,
            s,
            padding,
    ...
    def build_layer_hnet_conv2triv(
        self,
        r1: FieldType,
        C: int,
        s: int,
        ...
        if len(irreps) > 0:
            labels = [
                "trivial" if r.is_trivial() else "others" for r in layers[-1].out_type
            ]

            modules = []
            modules += [(ELU(trivials), "trivial")]
            modules += [
                (NormNonLinearity(others, function=function, bias=norm_bias), "others")
            ]
    ...
    def build_layer_gated_normpool(
        self,
        r1: FieldType,
        C: int,
        s: int,
        ...
        modules = [(ELU(trivials), "trivial"), (GatedNonLinearity1(gate), "gate")]
        nnl = MultipleModule(layers[-1].out_type, labels, modules)
        layers.append(nnl)
    ...
    def build_layer_gated_normpool_shared(
        self,
        r1: FieldType,
        C: int,
        s: int,
    ...
    def build_layer_gated_conv2triv_shared(
        self,
        r1: FieldType,
        C: int,
        s: int,
    ...
    def build_layer_inducedhnet_conv2triv(
        self,
        r1: FieldType,
        C: int,
        s: int,
        ...
        if self.fix_param and self.LAYER > 1 and not invariant_map:
            ...
            t *= sum(
                [
                    r.size**2 // r.sum_of_squares_constituents
                    for r in gc.fibergroup.irreps.values()
                ]
            )
            t /= 2 * F + 1
        ...
        if len(irreps) > 0:
            labels = [
                "trivial" if only_zero_freq(r) else "others" for r in layers[-1].out_type
            ]
    ...
    def build_layer_inducedgated_normpool(
        self,
        r1: FieldType,
        C: int,
        s: int,
    ...
    def build_layer_inducedgated_conv2triv(
        self,
        r1: FieldType,
        C: int,
        s: int,
