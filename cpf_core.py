...
class CPFcluster:
  ...
  def fit(self, X):
    ...
    if type(self.rho) in [list, np.ndarray]:
      self.labels_ = {}
      with Parallel(n_jobs= self.n_jobs) as parallel:
        labels_ = parallel(delayed(get_y)(self.CCmat, self.components, knn_radius, best_distance, big_brother, rho, self.alpha, d) for rho in self.rho)
      for i, rho in enumerate(self.rho):
        self.labels_[rho] = labels_[i]
    else:
